 			...
 			<div class="news_list">
 				...
 				<div>
 					<ul>
						<?php
	                        ...
	                        $all = mysql_query("SELECT * FROM article where state='1' and ( type='新闻' or 
	                        	type='公告' or type='活动' ) order by createTime desc limit $startCount,$pagesize "); //根据前面的计算出开始的记录和记录数
 				...
 				<div class="hide">
 					<ul>
 						 <?php
 						 	include '../manage/connect.php';
							$news = mysql_query("SELECT * FROM article where state='1' and type='新闻' order by createTime desc limit 0,13 ");
							while ($newsrow=mysql_fetch_array($news)) 
							{
								$t = strtotime($newsrow['createTime']);
								$time = date("Y/m/d",$t);
								echo "<li><a href='detail.php?id=".$newsrow['id']."'><em>[".$newsrow['type']."]</em>".$newsrow['title']."<span>".$time."</span></a></li>";
							}
						?>
 					</ul>
 				    <div>
 				...
 				<div class="hide">
 					<ul>
 						<?php
 							include '../manage/connect.php';
							$gg = mysql_query("SELECT * FROM article where state='1' and type='公告' order by createTime desc limit 0,13 ");
							while ($ggrow=mysql_fetch_array($gg)) 
							{
								$t = strtotime($ggrow['createTime']);
								$time = date("Y/m/d",$t);
								echo "<li><a href='detail.php?id=".$ggrow['id']."'><em>[".$ggrow['type']."]</em>".$ggrow['title']."<span>".$time."</span></a></li>";
							}
						?>
 					</ul>
 				    <div>
 				...
 				<div class="hide">
 					<ul>
 						 <?php
 						 	include '../manage/connect.php';
							$hd = mysql_query("SELECT * FROM article where state='1' and type='活动' order by createTime desc limit 0,13 ");
							while ($hdrow=mysql_fetch_array($hd)) 
							{
								$t = strtotime($hdrow['createTime']);
								$time = date("Y/m/d",$t);
								echo "<li><a href='detail.php?id=".$hdrow['id']."'><em>[".$hdrow['type']."]</em>".$hdrow['title']."<span>".$time."</span></a></li>";
							}
						?>
 					</ul>
 				    <div>

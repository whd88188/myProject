...
<div class="container">
	...
	<div class="allPicture" >
		...
		<table  class="table table-bordered table-hover">
			...
			<?php
				include 'connect.php';
				$result = mysql_query("SELECT * FROM banner  where type='scroll'  and type2='pc' order by sequance desc ");  //通过id倒序获取数据
				while($row= mysql_fetch_array($result))
				{
					echo "<tr>";
					echo "<td>" . $row['id'] . "</td>";
	...
	<div class="allPicture" >
		...
		<table  class="table table-bordered table-hover">
			...
			<tr style="display: none;" class="addBox2">
				...
				<td ><input type="text" name="sequance"></td> 
				<td ><input type="submit"  class="btn1" value="确认轮播图" ></td>
				<td colspan="2"><a href="#cancel" id="cancel2" class="btn">取消添加</a></td>
				</form>
			</tr>
		</table>
	</div>
	...
	<div class="allPicture" >
		...
		<table  class="table table-bordered table-hover">
			...
			<?php
				include 'connect.php';
				$result = mysql_query("SELECT * FROM banner where type='gl' order by sequance desc ");  //通过id倒序获取数据
				while($row= mysql_fetch_array($result))
				{
					echo "<tr>";
					echo "<td>" . $row['id'] . "</td>";
